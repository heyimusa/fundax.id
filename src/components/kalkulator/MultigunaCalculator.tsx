...
const MultigunaCalculator = () => {
  ...
  const [nilaiAset, setNilaiAset] = useState<number>(500000000);
  const [nilaiAsetText, setNilaiAsetText] = useState<string>((500000000).toLocaleString("id-ID"));
  ...
  const hitungMultiguna = React.useCallback(() => {
    try {
      ...
      // Untuk multiguna, plafon maksimal biasanya 70-80% dari nilai aset
      // Atau hitung berdasarkan kemampuan bayar (cicilan)
      let plafonBerdasarkanAset = nilaiAset * 0.7; // 70% dari nilai aset
      
      let plafonBerdasarkanCicilan = 0;
      if (!isBungaBergerak) {
        if (rFix > 0) {
          plafonBerdasarkanCicilan = jumlahCicilan * (1 - Math.pow(1 + rFix, -tenorBulan)) / rFix;
        } else {
          plafonBerdasarkanCicilan = jumlahCicilan * tenorBulan;
        }
      } else {
        plafonBerdasarkanCicilan = jumlahCicilan * tenorBulan;
      }

      // Ambil yang lebih kecil antara plafon berdasarkan aset atau cicilan
      const plafonAwal = Math.min(plafonBerdasarkanAset, plafonBerdasarkanCicilan);
      ...
      console.error("Error dalam perhitungan Multiguna:", error);
      setTabelAngsuran([]);
      setHasCalculated(false);
    }
  }, [pendapatanBulanan, usia, lamaPinjaman, jumlahCicilan, nilaiAset, sukuBungaFix, masaTahunFix, sukuBungaFloating, isBungaBergerak, sukuBungaBerjenjang]);
  ...
  const renderInputForm = () => (
    <div className="space-y-6">
      <div className="space-y-4">
        ...
        <div>
          <Label htmlFor="nilaiAset" className="text-sm font-normal text-gray-600">
          ...
          <Input
            ...
            onChange={(e) => {
              const formatted = formatWithDots(e.target.value);
              setNilaiAsetText(formatted);
              setNilaiAset(Math.max(0, parseDigits(formatted)));
            }}
            className="mt-1"
          />
        </div>
  ...
  const renderHasilPerhitungan = () => (
    <div className="space-y-6">
      ...
      {activeResultTab === "HASIL_KPR" && (
        <>
          <div className="mb-6">
            ...
            <p className="text-2xl font-bold text-fundax-blue">{formatCurrency(maksimalLimitPinjaman)}</p>
          </div>
          
          <div className="mb-6">
            <p className="text-sm text-gray-600">Nilai Aset yang Dijaminkan</p>
            <p className="text-lg font-semibold text-fundax-blue">{formatCurrency(nilaiAset)}</p>
            <p className="text-xs text-gray-500 mt-1">Plafon maksimal: {formatCurrency(nilaiAset * 0.7)} (70% dari nilai aset)</p>
          </div>
          ...
          onClick={async () => {
            ...
            const pdfData = {
              ...
              inputs: {
                ...
                nilaiAset: formatCurrency(nilaiAset),
              ...
              results: [
                ...
                {
                  title: 'Nilai Aset yang Dijaminkan',
                  value: formatCurrency(nilaiAset)
                },
                {
                  title: 'Plafon Maksimal (70%)',
                  value: formatCurrency(nilaiAset * 0.7)
                },
                {
      ...
      <p className="text-xs text-gray-500 mt-4">
        Catatan: Perhitungan ini adalah hasil perkiraan aplikasi Multiguna secara umum. Data perhitungan di atas dapat berbeda dengan perhitungan bank. Untuk perhitungan yang akurat, silakan hubungi bank penyedia pinjaman Multiguna.
      </p>
    </div>
  );
...
export default MultigunaCalculator; 